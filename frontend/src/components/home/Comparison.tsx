        ...
        <Container>
          ...
          <div className="mt-12 overflow-x-auto">
            <div className="min-w-[768px]">
              {/* Brand Logos */}
              <div className="grid grid-cols-5 gap-4 mb-8 min-w-[600px]">
                <div></div>
                {brands.map((brand, idx) => (
                  <div key={idx} className="flex flex-col items-center">
                    <div
                      className={`w-16 h-16 rounded-full flex items-center justify-center mb-3 border ${brand.name === 'Triksha' ? 'bg-[#4aa2f4]' : 'bg-slate-100 border-gray-300'}`}
                    >
                      <img src={brand.logo} alt={brand.name} className="h-10 w-10 object-contain" />
                    </div>
                    <div className="font-bold text-center text-sm">{brand.name}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-4">
                {features.map((feature, index) => (
                  ...
                  >
                    <div className="col-span-1 font-medium flex items-center text-sm md:text-base">
                      {feature.name}
                    ...
                    {[feature.triksha, feature.apple, feature.whoop, feature.ultrahuman].map((value, idx) => (
